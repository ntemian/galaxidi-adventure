...
async function main() {
  const lines = JSON.parse(fs.readFileSync(path.join(__dirname, "remaining-missing.json"), "utf8"));
  ...
  // Save results
  fs.writeFileSync(path.join(__dirname, "remaining-results.json"), JSON.stringify(results, null, 2));
  console.log(`\nDone! ${done} generated, ${failed} failed. Results in remaining-results.json`);
  ...
  const mapEntries = results
    ...
    .join("\n");
  fs.writeFileSync(path.join(__dirname, "remaining-voicemap.js"), mapEntries);
  console.log("VOICE_MAP entries saved to remaining-voicemap.js");
}
